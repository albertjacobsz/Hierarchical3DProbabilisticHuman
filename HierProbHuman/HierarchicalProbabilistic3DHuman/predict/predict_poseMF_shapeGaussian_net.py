...
def predict_poseMF_shapeGaussian_net(pose_shape_model,
    ...
    for image_fname in tqdm(sorted([f for f in os.listdir(image_dir) if f.endswith(('.jpg', '.png'))])):
        with torch.no_grad():
            ...
            torch.save(proxy_rep_img,'./edge_detect_output/'+image_fname.split('.')[0]+'_edge.pt')
            ...
            for i in range(0,n):
                f,u,s,v,pose,shape,glob,cam_wp = pose_shape_model(proxy_rep_input)
                if blob.shape[-1] == 3:
            ...
            pred_smpl_output_mode = smpl_model(body_pose=pred_pose_rotmats_mode,
                                               ...
                                               pose2rot=False)
            rm = pytorch3d.transforms.matrix_to_euler_angles(pred_glob_rotmats_mode)
        # -------------------------------------------- Angles in radians ---------------------------#
            print(rm)
        # --END--#
	    ...
	    # -------------------------------------------------PREDICTED  VERTICES ARE USED TO SHOW GREGY 3D model ------------------------------------------------------------------------#
            ...
            for x in ppf:
                    for rm in x:
                        ...
                        angY = math.atan2(-1*rm.item(6), math.sqrt(rm.item(7)**2 + rm.item(8)**2))
                        angZ = math.atan2(rm.item(3),rm.item(0))
                        
                        ecl = math.sqrt(rm.item(0)**2+rm.item(1)**2+rm.item(3)**2+rm.item(4)**2+rm.item(5)**2+rm.item(6)**2+rm.item(7)**2+rm.item(8)**2)
                        #print('ang x '+ str(angX))
                        #print('ang y '+str(angY))
                        #print('ang z '+ str(angZ))
                        df = [math.degrees(angX),math.degrees(angY),math.degrees(angZ)]
...
#---------------------------------- Adding all the data to a single array, preparing to write to CSV file 'all_data.csv' -----------------#
            ...
            DTA = [image_fname.split('.')[0], large_num, avg/den, count,camm.item(0),camm.item(1),camm.item(2), angX,angY,angZ]
...
def calculatePredS(pred_pose_S):
    ...
    for x in range(0,23):
        ...
        sDat.append(math.sqrt(float(pred_pose_S[0,x,0])**2+float(pred_pose_S[0,x,1])**2+float(pred_pose_S[0,x,2])**2))
